...
export const APIDocs = ({ children, lang }) => {
  ...
  return children.map(child => {
    ...
    if (inParameters) {
      if (child.props.mdxType === 'p') {
        inParameters = false;
        console.log('params children: ', child.props?.children, child.props?.children.length);
        ...
        return childStr.split("\n").map(line => {
          ...
          return parts.length > 1 ?
              <div key={`index_${i}`} style={{borderTop: "1px solid #ddd", paddingTop: 8, marginTop: 8}}><strong>{parts[0]}</strong>&nbsp;<span style={{color: "#888", fontSize: "small"}}>{parts[1]}</span></div>
            : <div key={`index_${i}`}>{parts[0]}</div>
        })
      }
    } else {
