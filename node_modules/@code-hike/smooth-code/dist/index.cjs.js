...
function useDimensions(code, focus, minColumns, lineNumbers, deps) {
    ...
    var prevLineRef = React__default['default'].useRef(null);
    var _b = React__default['default'].useMemo(function () {
        ...
        var element = (React__default['default'].createElement("code", { className: "ch-code-scroll-parent" },
            ...
            lines.map(function (line, i) { return (React__default['default'].createElement("div", { ref: line === prevLongestLine
                    ? prevLineRef
                    : undefined, key: i },
                lineNumbers ? (React__default['default'].createElement("span", { className: "ch-code-line-number" },
                    "_",
                    lineCount)) : undefined,
                React__default['default'].createElement("div", { style: { display: "inline-block" } },
...
function getLongestLine(code, focus) {
    ...
    var focusIndexes = utils.getFocusIndexes(focus, lines);
    var longestLine = "";
 ...
 */
function diff(code) {
    var changes = diff$1.diffLines(code.prev, code.next);
    var indexes = [];
    var prevIndex = 0;
    var nextIndex = 0;
...
function splitByFocus(mergedCode, focus, annotations) {
    ...
    var focusByLineNumber = utils.map(focus, function (focus) {
        return utils.mapFocusToLineNumbers(focus, lines);
    });
    var splittedLines = lines.map(function (line) {
        ...
        return __assign({ focused: utils.map(lineFocus, function (focus) { return !!focus; }), groups: getTokenGroups(tokens, lineFocus, lineAnnotations) }, rest);
    });
    var focusedLineNumbers = utils.map(focusByLineNumber, function (focusByLineNumber) {
        return Object.keys(focusByLineNumber).map(function (k) { return Number(k); });
    });
    var firstFocusedLineNumber = utils.map(focusedLineNumbers, function (focusedLineNumbers) { return Math.min.apply(Math, __spread(focusedLineNumbers)); });
    var lastFocusedLineNumber = utils.map(focusedLineNumbers, function (focusedLineNumbers) { return Math.max.apply(Math, __spread(focusedLineNumbers)); });
...
function getTokenGroups(tokens, focus, annotations) {
    var focusExtremes = utils.map(focus, function (focus) {
        return Array.isArray(focus) ? focus : [];
    });
    var annotationExtremes = utils.map(annotations, function (annotations) {
...
function getGroupElement(group) {
    return (React__default['default'].createElement(React__default['default'].Fragment, null, group.tokens.map(function (_a, i) {
        var content = _a.content, props = _a.props;
        return (React__default['default'].createElement("span", __assign({}, props, { key: i + 1 }), content));
    })));
}
/**
...
function getLinesWithElements(lines, verticalInterval, enterCount, exitCount) {
    ...
    return lines.map(function (line) {
        var lineIndex = utils.map(line.lineNumber, function (ln) { return ln && ln - 1; });
...
function parseAnnotations(annotations, theme) {
    ...
    var expandedAnnotations = utils.mapWithDefault(annotations, [], function (annotations) {
        return annotations.flatMap(function (annotation) {
            return utils.splitParts(annotation.focus).map(function (part) { return (__assign(__assign({}, annotation), { focus: part })); });
        });
    });
    var inlineCodeAnnotations = utils.mapWithDefault(expandedAnnotations, [], function (annotations) { return annotations.filter(isInline); });
    var multilineCodeAnnotations = utils.mapWithDefault(expandedAnnotations, [], function (annotations) { return annotations.filter(function (a) { return !isInline(a); }); });
    return {
        inlineAnnotations: utils.map(inlineCodeAnnotations, function (annotations) {
            return parseInlineAnnotations(annotations, theme);
        }),
        multilineAnnotations: utils.map(multilineCodeAnnotations, function (annotations) {
            return parseMultilineAnnotations(annotations, theme);
        }),
    };
}
function isInline(annotation) {
    return utils.hasColumns(annotation.focus);
}
function parseInlineAnnotations(annotations, theme) {
    var annotationMap = {};
    annotations.forEach(function (annotation) {
        var focusMap = utils.parsePartToObject(annotation.focus);
...
function defaultInlineComponent(annotation, theme) {
    return function (_a) {
        ...
        return (React__default['default'].createElement("span", { style: { outline: "red 1px solid" } }, children));
    };
}
function parseMultilineAnnotations(annotations, theme) {
    return annotations.map(function (annotation) {
        return {
            lineNumbers: utils.parseExtremes(annotation.focus),
            data: annotation.data,
            theme: theme,
            Component: annotation.Component ||
...
function defaultMultilineComponent(annotation, theme) {
    ...
    return function (_a) {
        ...
        return (React__default['default'].createElement("div", { style: __assign(__assign({}, style), { background: bg, cursor: "pointer" }), onClick: function (_) { return alert("clicked"); }, className: "ch-code-bg-annotation" },
...
function useStepParser(input) {
    ...
    return React__default['default'].useMemo(function () { return parse(input); }, [
...
function parse(_a) {
    ...
    var focusedCode = splitLinesByFocus(mergedCode, utils.withDefault(focus, null), inlineAnnotations);
...
function getCode(highlightedLines) {
    return utils.map(highlightedLines, function (lines) {
        return lines
...
function addExtraStuff(codeStep, code) {
    ...
    var newGroups = utils.map(codeStep.lineGroups, function (groups) {
...
function SmoothContainer(_a) {
    ...
    return (React__default['default'].createElement(Container, { width: dimensions.containerWidth, height: dimensions.containerHeight },
        React__default['default'].createElement(Content, { dx: dx, dy: dy, scale: zoom, height: Math.max(focusHeight, dimensions.containerHeight), width: width }, children(focusWidth, startX))));
}
...
function getTweenContentProps(_a) {
    ...
    return utils.map(paramTween, function (_a) {
...
function SmoothLines(props) {
    return (React__default['default'].createElement(SmoothContainer, __assign({}, props), function (focusWidth, startX) { return (React__default['default'].createElement(Lines, { codeStep: props.codeStep, focusWidth: focusWidth, lineHeight: props.dimensions.lineHeight, progress: props.progress, theme: props.theme, startX: startX, lineNumberWidth: props.dimensions.lineNumberWidth })); }));
}
function Lines(_a) {
    ...
    return (React__default['default'].createElement(React__default['default'].Fragment, null, groups.map(function (group, i) {
        if (!group.annotation) {
...
function LineGroup(_a) {
    ...
    return (React__default['default'].createElement(React__default['default'].Fragment, null, lines.map(function (line, key) {
        ...
        return (React__default['default'].createElement(React__default['default'].Fragment, { key: key },
            lineNumberWidth ? (React__default['default'].createElement("span", { className: "ch-code-line-number", style: {
                    ...
                    opacity: opacity,
                    color: utils.getColor(theme, utils.ColorName.LineNumberForeground),
                } }, t < 0.5
...
function LineContent(_a) {
    ...
    return (React__default['default'].createElement("div", { style: {
        ...
        line.annotatedGroups.map(function (annotatedGroup, i) { return (React__default['default'].createElement(AnnotatedTokens, { annotatedGroup: annotatedGroup, progress: progress, dx: dx, key: i })); }),
        React__default['default'].createElement("br", null)));
}
function AnnotatedTokens(_a) {
    ...
    var children = tokenGroups.map(function (group, i) {
        ...
        return (React__default['default'].createElement("span", { style: { opacity: opacity }, key: i + 1 }, group.element));
    });
    return Component ? (React__default['default'].createElement(Component, { children: children, data: (_c = annotated === null || annotated === void 0 ? void 0 : annotated.annotation) === null || _c === void 0 ? void 0 : _c.data, theme: (_d = annotated === null || annotated === void 0 ? void 0 : annotated.annotation) === null || _d === void 0 ? void 0 : _d.theme })) : (React__default['default'].createElement(React__default['default'].Fragment, null, children));
}
...
function useCodeShift(_a) {
    ...
    return useStepParser({
        highlightedLines: utils.map(tween, function (tween) { return tween.code.lines; }),
        theme: theme,
        focus: utils.map(tween, function (tween) { return tween.focus; }),
        annotations: utils.map(tween, function (tween) { return tween.annotations; }),
    });
}
...
function CodeTween(_a) {
    ...
    var _b = useDimensions(stepInfo.code, utils.map(tween, function (tween) { return tween.focus; }), config.minColumns || DEFAULT_MIN_COLUMNS, config.lineNumbers || false, [config.parentHeight]), element = _b.element, dimensions = _b.dimensions;
    // return (
    //   <BeforeDimensions
    //     element={element}
    //     htmlProps={preProps}
    //   />
    // )
    return !dimensions ? (React__default['default'].createElement(BeforeDimensions, { element: element, htmlProps: preProps })) : (React__default['default'].createElement(AfterDimensions, { dimensions: dimensions, stepInfo: stepInfo, config: config, progress: progress, htmlProps: preProps }));
}
function BeforeDimensions(_a) {
    var element = _a.element, htmlProps = _a.htmlProps;
    return (React__default['default'].createElement(Wrapper, { htmlProps: htmlProps, 
...
function CodeSpring(_a) {
    ...
    return (React__default['default'].createElement(CodeTween, __assign({ tween: tween, progress: t, config: config }, htmlProps)));
}
